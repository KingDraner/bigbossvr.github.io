...
if ( ! function_exists( 'point_setup' ) ) :
	...
	function point_setup() {
		...
		*/
		...
		add_action( 'admin_init', 'point_notice_ignore_rm' );
		function point_notice_ignore_rm() {
			global $current_user;
			$user_id = $current_user->ID;
			/* If user clicks to ignore the notice, add that to their user meta */
			if ( isset( $_GET['point_notice_ignore'] ) && '0' === $_GET['point_notice_ignore'] ) {
				add_user_meta( $user_id, 'point_ignore_notice_rm', 'true', true );
			}
		}
		...
		function point_pro_banner() {
			echo '<a href="https://mythemeshop.com/themes/point/?utm_source=Point+Free&utm_medium=Banner+CPC&utm_content=Point+Pro+LP&utm_campaign=WordPressOrg" id="pro-banner" style="display: none; margin-top: 10px; background: #fff;" target="_blank"><img src="' . esc_url( get_template_directory_uri() ) . '/images/point-pro.jpg" /></a>';
